...
public interface BaseDeleteReqController<REQ, T, ID> extends DeleteLogic<T,ID>,
		                                                             ReqMapper<REQ,T>
{
    ...
    default ResponseEntity<REQ> deleteAsReq(@PathVariable ID idValue) throws DataAccessException {
        T deletedEntity = getDeletedEntity(idValue);
        return ResponseEntity.status(HttpStatus.ACCEPTED).body(toREQ(deletedEntity));
    }
